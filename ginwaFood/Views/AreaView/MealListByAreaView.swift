...
struct MealListByAreaView: View {
    ...
    var body: some View {
        ZStack {
            VStack {
                ...
                .navigationTitle(rowModel?.strArea ?? "")
    ...
    private func loadData() async{
        do {
            let appendUrl =  (rowModel?.strArea ?? "")
                .addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
            guard let url =
                    URL(string: "https://www.themealdb.com/api/json/v1/1/filter.php?a=" + appendUrl)
            else {
...
struct MealListByAreaView_Previews: PreviewProvider {
    static var previews: some View {
        MealListByAreaView(rowModel: AreaRowModel(strArea: "American"))
    }
}
